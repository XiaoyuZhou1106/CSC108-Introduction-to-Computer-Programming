...
def format_data(data: List[List[str]]) -> None:  
    ...
    for lis in range(len(data)):
        ...
        data[lis][ID_INDEX] = lis + 1
        data[lis][LAT_INDEX] = float(data[lis][LAT_INDEX])
        data[lis][LON_INDEX] = float(data[lis][LON_INDEX])
        data[lis][NUM_SPANS_INDEX] = int(data[lis][NUM_SPANS_INDEX])
        
        for e in range(len(data[lis][SPAN_LENGTH_INDEX])):
            if data[lis][SPAN_LENGTH_INDEX][e] == '=':
                equal.append(e)
            elif data[lis][SPAN_LENGTH_INDEX][e] == ';':
                semi.append(e)
        equal.remove(equal[0])
        
        for f in range(len(equal)):
            spanl.append(float(data[lis][SPAN_LENGTH_INDEX][equal[f] + 1:semi[f]]))
        data[lis][SPAN_LENGTH_INDEX] = spanl                                        
        
        data[lis][LENGTH_INDEX] = float(data[lis][LENGTH_INDEX])
        
        bcisa = data[lis][BCIS_INDEX:]
        for j in bcisa:
            if j == '':
                bcisa.remove(j)
        for k in bcisa:    
            k = float(k)
        bcisa.remove(bcisa[0])
        data[lis][BCIS_INDEX:] = [bcisa]
...
def get_bridge(bridge_data: List[list], bridge_id: int) -> list:
    ...
    True
    """
    if bridge_id <= len(bridge_data):
        result = bridge_data[bridge_id - 1]
    else:
        result = ''
...
def get_average_bci(bridge_data: List[list], bridge_id: int) -> float:
    ...
    if bridge_id <= len(bridge_data) and len(bridge_data[bridge_id - 1]
                                             [BCIS_INDEX]) > 0:
        for n in bridge_data[bridge_id - 1][BCIS_INDEX]:
            a = a + n
        avg = a / len(bridge_data[bridge_id - 1][BCIS_INDEX])
    else:
        avg = 0.0
...
def get_total_length_on_highway(bridge_data: List[list], highway: str) -> float:
    ...
    for m in range(len(bridge_data)):
        if bridge_data[m][HIGHWAY_INDEX] == highway:
            length = length + bridge_data[m][LENGTH_INDEX]
            b = b + 1
...
def get_distance_between(bridge1: list, bridge2: list) -> float:
    ...
    dis = calculate_distance(bridge1[LAT_INDEX], bridge1[LON_INDEX], 
...
def find_closest_bridge(bridge_data: List[list], bridge_id: int) -> int:
    ...
    close = []
    br2 = get_bridge(THREE_BRIDGES, bridge_id)
    con = 0
    
    for z in range(len(bridge_data)):
        br1 = get_bridge(THREE_BRIDGES, z + 1)
        if z != bridge_id - 1:
            close.append(calculate_distance(br1[LAT_INDEX], br1[LON_INDEX], 
                                            br2[LAT_INDEX], br2[LON_INDEX]))
    
    clostest = min(close)
    
    for x in range(len(bridge_data)):
        br3 = get_bridge(THREE_BRIDGES, x + 1)
        if calculate_distance(br3[LAT_INDEX], br3[LON_INDEX], br2[LAT_INDEX], 
                              br2[LON_INDEX]) == clostest:
            con = x + 1
...
def find_bridges_in_radius(bridge_data: List[list], lat: float, long: float,
    ...
    for y in range(len(bridge_data)):
        br4 = get_bridge(THREE_BRIDGES, y + 1)
        if calculate_distance(br4[LAT_INDEX], 
                              br4[LON_INDEX], lat, long) <= distance:
            inradius.append(y + 1)
...
def get_bridges_with_bci_below(bridge_data: List[list], bridge_ids: List[int],
    ...
    for w in bridge_ids:
        if bridge_data[w - 1][BCIS_INDEX][0] <= bci_limit:
            bcibelow.append(w)
...
def get_bridges_containing(bridge_data: List[list], search: str) -> List[int]:
    ...
    for v in range(len(bridge_data)):
        if search.lower() in bridge_data[v][NAME_INDEX].lower():
            contain.append(v + 1)
...
def assign_inspectors(bridge_data: List[list], inspectors: List[List[float]],
    ...
    for u in range(len(bridge_data)):    
        if HIGH_PRIORITY_BCI < bridge_data[u][BCIS_INDEX][0] <= MEDIUM_PRIORITY_BCI:
            mediump.append(u + 1)
        elif MEDIUM_PRIORITY_BCI < bridge_data[u][BCIS_INDEX][0] <= LOW_PRIORITY_BCI:
            lowp.append(u + 1)
        elif bridge_data[u][BCIS_INDEX][0] <= HIGH_PRIORITY_BCI:
            highp.append(u + 1)
    
    for t in range(len(inspectors)):
        highr.extend(find_bridges_in_radius(bridge_data, inspectors[t][0],
                                            inspectors[t][1], 
                                            HIGH_PRIORITY_RADIUS))
        mediumr.extend(find_bridges_in_radius(bridge_data, inspectors[t][0], 
                                              inspectors[t][1], 
                                              MEDIUM_PRIORITY_RADIUS))
        lowr.extend(find_bridges_in_radius(bridge_data, inspectors[t][0], 
                                           inspectors[t][1], 
                                           LOW_PRIORITY_RADIUS))
    ...
    for p in range(len(inspectors)):
        final_assign.append(allf[max_bridges * p:max_bridges * (p + 1)])
...
def inspect_bridges(bridge_data: List[list], bridge_ids: List[int], date: str, 
    ...
    bridge_data[bridge_ids[0] - 1][LAST_INSPECTED_INDEX] = date
    bridge_data[bridge_ids[0] - 1][BCIS_INDEX].insert(0, bci)
